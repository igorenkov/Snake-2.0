...
typedef struct Snake {
	...
	char direct; //Символ головы змейки
	int move_x; //Направление движения головы
	int move_y;
	int life; //Что-то вроде индекса
	char up; //Кнопки движения на клавиатуре
	char down;
	char left;
	char right;
} Snake;
...
typedef struct Field {
	...
	int apples; //Количество не собранных яблок
} Field;
...
void move(Snake* snake1, Snake* snake2, Field* field) {
	...
	int flag = 0;
	char temp;
	if (snake1->head.col == 1 && snake1->head.row == 0) {	//условие на первое движение
		while (!flag) {
			if (_kbhit()) {
				temp = _getch();
		...
		else if (temp == -32) { //Первой была нажата стрелка
		...
		else if (temp == 'w' || temp == 's' || temp == 'a' || temp == 'd') {	//Первой была нажата клавиша
			move_snake(snake1, temp);
			while (_kbhit()) {
				temp = _getch();
				if (temp == -32) {
...
//Функция рисования символа symb в точке (x, y) с цветом
...
int main() {
	...
	while (1) {
		...
		else if (game_over == 1) {	//Один врезался, другой дошёл до финиша
			clean_string(field, 2);
			clean_string(field, 3);
			clean_string(field, 4);
			clean_string(field, 1);
